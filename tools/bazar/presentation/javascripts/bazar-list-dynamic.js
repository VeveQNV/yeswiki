	...
	document.querySelectorAll(".bazar-list-dynamic-container").forEach (domElement =>
	{
		var vVue = new Vue
	  	({
			...
			watch: {
				geolocateds : function () {this.filterEntries()  },
				passeds : function () {this.filterEntries()  },
			...
			methods: {
			  ...
			  filterEntries() {
				...
				result = result.filter(entry => {
				 ...
				 if (parseFloat (entry["bf_distance"]) == "-1" && this.geolocateds) return false; else if (this.passeds && entry["bf_date_fin"] && Date.parse(entry["bf_date_fin"]) < Date.now ()) return false; else return true;})
	 	 ...
	 	 wiki.vuejs.register ($(vVue.$el), vVue);
	})
})
